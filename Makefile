...
on-app-prod: ## On app in production mode
	@echo "List charts TAG image exits on your computer:"
	@docker images --format "- {{.Tag}}" charts-app
	@read -p "Run charts app with docker image TAG [latest]: " TAG; \
	set -eux; \
	docker-compose up -d charts_db; \
...
build-prod: ## Docker build charts production image
	@printf "To run in non-interactive mode please run command bellow:\n\t";\
	printf "${TITLE_COLOR}TAG=charts_tag AWS_ACCOUNT_ID=your_aws_account make build-prod-fargate${NO_COLOR}\n";\
	\
	if [ -z "$$TAG" ]; then\
		read -p "charts docker image TAG [`date +'%Y.%m.%d'`] TAG=" TAG;\
	fi;\
	\
	...
	TAG=$${TAG:=`date +'%Y.%m.%d'`};\
	printf "${TITLE_COLOR}Building charts with TAG='$$TAG' and push to aws account AWS_ACCOUNT_ID='$$AWS_ACCOUNT_ID'${NO_COLOR} ...\n";\
	\
	...
	\
	docker tag charts-public:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/charts-public:$${TAG};\
	docker tag charts-app:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/charts-app:$${TAG};\
...
docker-push: ## Docker push faragte image. Make sure you run docker-login first
	@if [ -z "$$AWS_ACCOUNT_ID" -o -z "$$TAG" ]; then\
		printf "To push charts fargate image please run:\n";\
		printf "${TITLE_COLOR}AWS_ACCOUNT_ID=x TAG=y make docker-push${NO_COLOR}\n";\
	else\
		docker push $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/charts-public:$${TAG};\
		docker push $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/charts-app:$${TAG};\
	fi
...
test: on-app## Run phpunit test app integration with database
	docker exec -t charts_app tests/phpunit --no-charts --testdox -c tests/phpunit.xml

test-library: on-app ## Run phpunit test library only - testdox
	docker exec -t charts_app tests/phpunit --no-charts --testdox -c tests/phpunit.lib.xml

test-charts: on-app ## Measure test charts
	docker exec -t -e XDEBUG_MODE=charts charts_app tests/phpunit -d memory_limit=-1 -c tests/phpunit.xml

clean-test-charts: ## Clean test charts results
	rm -rf tests/charts

ssh-app: ## SSH into charts app container
	docker exec -it charts_app sh

ssh-app-as-root: ## SSH into charts app container as root user
	docker exec -it --user 0 charts_app sh

show-log-app: ## Show log charts app container
	docker logs -f charts_app

show-log-app-nginx: ## Show log charts nginx container
	docker logs -f charts_public

show-log-db: ## Show log mysql server container
	docker logs -f charts_db

# show-log-saml: ## Show log saml server - keycloak container
# 	docker logs -f keycloak
npm-run: ## Run NPM run in APP container
	docker exec -t --user 0 charts_app npm run watch

check-eslint: ## Run check eslint in source
	docker exec -t --user 0 charts_app npm run eslint


composer-install: ## Run Composer install in APP container
	docker exec -t charts_app composer install

npm-install: ## Run NPM install in APP container
	rm -rf node_modules && docker exec -t --user 0 charts_app npm install

up-db: ## Migrate database for all tenant
	docker exec -it --user 0 charts_app sh php artisan migrate
...
# 		docker-compose exec charts_db mysql -pcdmllove $$dbname -e "update crm_member set member_used_2auth=0 where member_id=1;";\
# 	done
#
